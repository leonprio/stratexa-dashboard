...
import { Dashboard as DashboardType, DashboardItem, DashboardRole, SystemSettings, GlobalUserRole, User, ManagedClient } from './types';
...
import { StrategicPerspective, StrategicObjective, AreaStrategyConfig, ContributionObjective, ContributionIndicatorAssignment, StrategicObjectiveRelationship } from './strategyTypes';
...
export default function App() {
  ...
  const loadStrategyData = useCallback(async () => {
    ...
    try {
      const [pList, oList, acList, coList, asgnList, relList] = await Promise.all([strategyService.getPerspectives(selectedClientId), strategyService.getStrategicObjectives(selectedClientId), strategyService.getAreaConfigs(selectedClientId), strategyService.getContributionObjectives(selectedClientId), strategyService.getAssignments(selectedClientId), strategyService.getStrategicObjectiveRelationships(selectedClientId)]);
  ...
  const handleSaveRelationship = useCallback(
    async (rel: { sourceStrategicObjectiveId: string; targetStrategicObjectiveId: string; description?: string }) => {
  ...
  const isGlobalAdmin = useMemo(() => userProfile?.globalRole === GlobalUserRole.Admin || userProfile?.globalRole?.toLowerCase() === 'admin', [userProfile]);

  const isDirector = useMemo(() => userProfile?.globalRole === GlobalUserRole.Director || userProfile?.globalRole?.toLowerCase() === 'director', [userProfile]);
  ...
  const officialGroups = useMemo(() => {
    ...
    if (!isGlobalAdmin && userProfile) {
      ...
      const userIsOps = userProfile.directorTitle?.toUpperCase().includes('OPERACIONES') || userProfile.group?.toUpperCase().includes('OPERACIONES') || userProfile.subGroups?.some((sg) => sg.toUpperCase().includes('OPERACIONES'));
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        const isAdminEmail = normalizedEmail.includes('leon@leonprior.com') || normalizedEmail.includes('leonprior@gmail.com');
      ...
      try {
        ...
        const processedRows = rows.map((r) => {
          if (!r.group && (r.clientId || 'IPS').toLowerCase() === 'ips') {
            ...
            const stdMatch = IPS_DASHBOARDS.find((std) => (r.title || '').trim().toUpperCase() === std.name.trim().toUpperCase() || (r.title || '').trim().toUpperCase().includes(std.name.trim().toUpperCase()));
        ...
        const enrichedRows = filteredRows.map((r) => {
          ...
          const distinctDirectors = allUsers.filter(
            (u) =>
              ...
              u.dashboardAccess &&
              (u.dashboardAccess[r.id] || (r.originalId && u.dashboardAccess[r.originalId]))
          );
        ...
        const groupsToAggregate = userProfile?.subGroups && userProfile.subGroups.length > 0 ? userProfile.subGroups : localOfficialGroups;
        ...
        groupsToAggregate.forEach((gName) => {
          ...
          if (groupBoards.length > 0) {
            ...
            if (!userProfile?.subGroups?.length) {
              const director = allUsers.find((u) => (u.globalRole === 'Director' || u.globalRole === 'Admin') && (u.clientId || '').trim().toUpperCase() === currentClientAgg && (normalizeGroupName(u.directorTitle) === normGName || normalizeGroupName(u.group) === normGName));
            ...
            const dominantArea = isHierarchyRoot ? '' : areaCounts.size > 0 ? Array.from(areaCounts.entries()).sort((a, b) => b[1] - a[1])[0][0] : normGName;
        ...
        const hierarchyDirectors = allUsers.filter((u) => u.globalRole === 'Director' && (u.clientId || 'IPS').trim().toUpperCase() === currentClientAgg && u.subGroups && u.subGroups.length > 0);
  ...
  const handleFixOrder = async (currentList?: DashboardType[]) => {
    try {
      ...
      const relevantDashboards = sourceList.filter((d) => !String(d.id).startsWith('agg-') && d.id !== -1 && d.clientId?.trim().toUpperCase() === clientTarget && d.year === selectedYear).sort((a, b) => (Number(a.orderNumber) || 0) - (Number(b.orderNumber) || 0));
    ...
    try {
      ...
      if (applyGlobally) {
        ...
        const allAreas = [
          ...new Set(
            allRawDashboards
              .filter((d) => !String(d.id).startsWith('agg-') && d.id !== -1 && String(d.clientId || 'IPS').toUpperCase() === targetClient && Number(d.year || currentYear) === Number(currentYear))
      ...
      <header className="sticky top-0 z-50 flex flex-col md:flex-row items-center justify-between gap-2 bg-slate-950/90 py-1.5 px-6 rounded-b-2xl border-b border-white/5 backdrop-blur-3xl">
        ...
        <div className="flex flex-row items-center gap-4 shrink-0">
          <div className="flex flex-col items-start">
            <div className="flex items-center gap-2">
              <h1 className="text-xl lg:text-2xl font-black text-white italic uppercase tracking-tighter leading-none">{selectedClientId === 'all' || !selectedClientId ? 'TABLERO GLOBAL' : (availableManagedClients.find((c) => c.clientId === selectedClientId.toUpperCase())?.displayName || selectedClientId).toUpperCase()}</h1>
              {isGlobalAdmin && selectedClientId && selectedClientId !== 'all' && (
                <button onClick={handleRenameClient} className="p-1.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-cyan-400 transition-all scale-75" title="Renombrar Cliente">
                  ✏️
                </button>
              )}
            </div>
        ...
        <div className="flex flex-wrap items-center justify-center md:justify-end gap-2 scale-[0.85] origin-right">
          <div className="flex bg-black/40 p-0.5 rounded-xl border border-white/5">
            <select value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))} className="bg-transparent text-white px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest outline-none border-none">
          ...
          {(isGlobalAdmin || userProfile?.canManageKPIs) && (
            <nav className="flex items-center gap-0.5 bg-black/40 p-0.5 rounded-xl border border-white/5 overflow-x-auto">
              {isGlobalAdmin && (
                <>
                  <button onClick={() => setActiveAdminSection('master')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'master' ? 'bg-purple-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`} title="Semáforo Maestro Multi-Cliente">
                    Global
                  </button>
                  {/* <button
                  ...
                  <button onClick={() => setActiveAdminSection('users')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'users' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                    Usuarios
                  </button>
                </>
              )}
              ...
              {isGlobalAdmin && (
                <button onClick={() => setActiveAdminSection('weights')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'weights' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`} title="Ponderación de Áreas / Sucursales para el total global">
                  Pesos Tablero
                </button>
              )}
              ...
              {isGlobalAdmin && (
                <>
                  <button onClick={() => setActiveAdminSection('thresholds')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'thresholds' ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                    Semáforos
                  </button>
                  <button onClick={() => setActiveAdminSection('clients')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'clients' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                    Config
                  </button>
                  <button onClick={() => setActiveAdminSection('import')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'import' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                    CSV
                  </button>
                </>
              )}
              ...
              {settings?.enableStrategyMap && (
                <button onClick={() => setActiveAdminSection('strategy')} className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeAdminSection === 'strategy' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-950/40' : 'text-slate-400 hover:text-white hover:bg-white/5'}`} title="Matriz de Contribución Estratégica">
                  Estrategia
                </button>
              )}
            </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-slate-900 border-2 border-white/5 rounded-2xl px-2">
              <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-transparent py-2.5 px-2 text-xs font-black text-white outline-none cursor-pointer uppercase tracking-widest">
            ...
            <div className="flex bg-slate-900 border border-white/5 rounded-2xl p-1">
              <button onClick={() => setViewMode('grid')} className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white'}`} title="Vista Normal">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                </svg>
              </button>
              <button onClick={() => setViewMode('compact')} className={`p-2 rounded-xl transition-all ${viewMode === 'compact' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-500 hover:text-white'}`} title="Vista Minimalista">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button onClick={() => setActiveAdminSection('help')} className="bg-white/5 hover:bg-indigo-500/20 text-slate-400 hover:text-indigo-400 border border-white/5 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all" title="Manual de Vuelo (Instrucciones)">
              ❓ Ayuda
            </button>

            <button onClick={handleLogout} className="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">
              Salir
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setActiveAdminSection('none')}>
          <div className="bg-slate-900 border border-white/10 rounded-[2.5rem] w-full max-w-lg p-10 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleDownloadBackup} className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-slate-300 font-black rounded-2xl shadow-xl transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-3 border border-white/5">
                <span>💾</span> Respaldo Completo (.JSON)
              </button>

              <button onClick={() => setActiveAdminSection('none')} className="w-full py-4 bg-transparent hover:bg-white/5 text-slate-500 font-bold rounded-2xl transition-all uppercase tracking-widest text-[10px]">
                Cancelar
              </button>
            </div>
          </div>
...
Esto corregirá cualquier inconsistencia en colores (ej. Amarillo vs Rojo).`)
      ...
      {activeAdminSection === 'clients' && <ClientSettings dashboards={dashboards} selectedClientId={selectedClientId} selectedYear={selectedYear} setActiveAdminSection={(s) => setActiveAdminSection(s as any)} handleFixOrder={handleFixOrder} handleDownloadBackup={handleDownloadBackup} settings={settings} handleUpdateSystemSettings={handleUpdateSystemSettings} setLoadingDashboards={setLoadingDashboards} allRawDashboards={allRawDashboards} currentUser={userProfile || undefined} />}
      ...
      {activeAdminSection === 'help' && <HelpCenter userRole={userProfile?.globalRole || 'Member'} onClose={() => setActiveAdminSection('none')} />}
      ...
      {loadingDashboards ? (
        ...
        <div className="py-24 text-center bg-slate-900/20 border-2 border-dashed border-white/5 rounded-[3rem] p-12">
          ...
          {isGlobalAdmin && (
            <div className="flex flex-col items-center gap-4">
              <button onClick={handleAddDashboard} className="group relative px-10 py-5 bg-cyan-600 hover:bg-cyan-500 text-white font-black rounded-3xl shadow-3xl shadow-cyan-900/40 transition-all active:scale-[0.98] border border-white/20 overflow-hidden w-full max-w-md">
              ...
              <button onClick={() => setActiveAdminSection('clients')} className="group relative px-10 py-4 bg-slate-900 hover:bg-slate-800 text-slate-400 font-bold rounded-2xl border border-white/5 transition-all active:scale-[0.98] w-full max-w-md">
                <div className="flex items-center gap-3 justify-center">
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-4">Define el nombre de tu departamento o sucursal inmediatamente</p>
            </div>
          )}
        </div>
      ) : (
        <div className="flex flex-col md:flex-row gap-0 overflow-hidden min-h-[calc(100vh-120px)]">
          ...
          <HierarchySidebar dashboards={dashboards} selectedDashboardId={selectedDashboardId} onSelectDashboard={setSelectedDashboardId} settings={settings} isGlobalAdmin={isGlobalAdmin} isDirector={isDirector} isCollapsed={isSidebarCollapsed} onToggleCollapse={handleToggleSidebar} onAddDashboard={isGlobalAdmin ? handleAddDashboard : undefined} onDeleteDashboard={isGlobalAdmin ? handleDeleteDashboard : undefined} allUsers={allUsers} userProfile={userProfile} selectedClientId={selectedClientId} />
          ...
          <main className="flex-grow min-w-0 px-4 lg:px-8 py-1 overflow-y-auto">
            {selectedDashboard && (
              <div className="flex items-center gap-2 mb-2 text-[8px] font-black uppercase tracking-[0.15em] text-slate-500 overflow-x-auto whitespace-nowrap scrollbar-hide py-1.5 px-4 bg-slate-900/40 rounded-xl border border-white/5 shadow-lg backdrop-blur-md">
                ...
                {/* 🛡️ NIVEL ACTUAL */}
                <span className="text-white border-b-2 border-cyan-500/80 pb-0.5 font-black tracking-tight drop-shadow-[0_0_8px_rgba(34,211,238,0.3)]">{selectedDashboard.title}</span>
              </div>
            )}
            ...
            {selectedDashboard ? (
              ...
              <div className="py-24 text-center text-slate-500 font-bold uppercase tracking-widest text-xs border border-white/5 rounded-[2rem] bg-slate-900/20">← Seleccione un elemento del panel de navegación</div>
            )}
          </main>
        </div>
      )}
