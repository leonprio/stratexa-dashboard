...
export const RescheduledCommitmentsSection: React.FC<{ commitments: RescheduledKpiCommitment[]; onManage: (commitment: RescheduledKpiCommitment) => void; renderManager?: (commitment: RescheduledKpiCommitment) => React.ReactNode }> = ({ commitments, onManage, renderManager }) => commitments.length === 0 ? null : <section className="rounded-xl border border-cyan-500/20 bg-slate-950/40 p-3"><h3 className="text-[9px] font-black uppercase tracking-widest text-cyan-300">COMPROMISOS REPROGRAMADOS ({commitments.length})</h3>{commitments.map(commitment => <React.Fragment key={commitment.id}><div className="mt-2 flex items-center justify-between gap-3"><div><p className="text-xs text-slate-200">{commitment.label}</p><p className="text-[10px] text-slate-500">Origen: {commitment.periodLabel} → Compromiso: {commitment.scheduledPeriodLabel}</p><div className="mt-1 flex gap-1"><span className="rounded border border-cyan-500/30 px-1.5 py-0.5 text-[8px] font-black text-cyan-300">REPROGRAMADA</span><span className="rounded border border-slate-600 px-1.5 py-0.5 text-[8px] font-black text-slate-400">NO SUMA A META</span></div></div><button type="button" onClick={() => onManage(commitment)} className="rounded-lg border border-cyan-500/30 px-3 py-2 text-[9px] font-black text-cyan-300">GESTIONAR</button></div>{renderManager?.(commitment)}</React.Fragment>)}</section>;
