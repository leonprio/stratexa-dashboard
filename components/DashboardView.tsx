...
export const DashboardView: React.FC<DashboardViewProps> = React.memo(
  ({
    ...
    const safeItems: DashboardItem[] = useMemo(
      () => dashboard.items ?? [],
      [dashboard],
    );
    const strategicallyOrderedItems = useMemo(
      () =>
    ...
    useEffect(() => {
      ...
      if (
        safeItems.some((item) => String(item.id) === String(requestedItemId))
      ) {
        setNavigationError("");
    ...
    const isAggregate =
      (typeof dashboard.id === "string" && dashboard.id.startsWith("agg-")) ||
      dashboard.id === -1 ||
    ...
    const [localDecimalPrecision, setLocalDecimalPrecision] = useState<
      0 | 1 | 2
    >(0);
    ...
    const activeThresholds = useMemo(() => {
      return (
        ...
        settings?.thresholds || { onTrack: 95, atRisk: 85 }
      );
    }, [dashboard, settings]);

    const totalScore = useMemo(() => {
      return calculateDashboardWeightedScore(
        safeItems,
        activeThresholds,
        year,
        "realTime",
    ...
    const handleActionPlanExit = useCallback(() => {
      setSelectedItemId(null);
      setActiveView("objectives");
      setReturnContext("plans");
      onActionPlanExit?.();
      scheduleScroll(() => scrollToTop());
    }, [onActionPlanExit]);
    ...
    const displayTitle = useMemo(() => {
      ...
      if (
        isAggregate ||
        t.toUpperCase().includes("SÍNTESIS") ||
        t.toUpperCase().includes("RESUMEN")
      ) {
    ...
    const diag = useMemo(() => {
      ...
      return {
        ...
        keys,
        hasItemsByMonth: Object.prototype.hasOwnProperty.call(
          d,
          "itemsByMonth",
        ),
    ...
    const canExportExecutive = useMemo(() => {
      ...
      if (isDirector) {
        ...
        if (currentUser?.group)
          allowedGroups.add(currentUser.group.trim().toUpperCase());
        if (currentUser?.subGroups)
          currentUser.subGroups.forEach((g) =>
            allowedGroups.add(g.trim().toUpperCase()),
          );
        if (currentUser?.superGroups)
          currentUser.superGroups.forEach((g) =>
            allowedGroups.add(g.trim().toUpperCase()),
          );
      ...
      const isAreaMatching =
        currentUser?.group &&
        dashboard.area &&
        dashboard.area.trim().toUpperCase() ===
          currentUser.group.trim().toUpperCase();
    ...
    const handleExportExecutiveExcelJS = async () => {
      try {
        ...
        alert(
          "❌ Error al generar el reporte operativo ejecutivo: " + err.message,
        );
      } finally {
    ...
    return (
      ...
      >
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                <h2
                  ...
                  title={
                    isGlobalAdmin
                      ? `Tablero #${(dashboard as any).orderNumber || "N/A"}`
                      : undefined
                  }
                >
                  {displayTitle}
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded-full ${isAggregate ? "bg-indigo-500 shadow-[0_0_8px_#6366f1]" : "bg-cyan-500 shadow-[0_0_8px_#22d3ee]"}`}
                  />
          ...
          <div className="flex flex-wrap items-center justify-start lg:justify-end gap-2 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-3 glass-panel px-4 py-1.5 rounded-xl shadow-xl border border-white/5">
              <div className="flex flex-col items-end">
                ...
                {hasGoals ? (
                  <span
                    className={`text-2xl sm:text-3xl font-black tabular-nums leading-none ${totalStatus === "OnTrack" ? "text-emerald-400" : totalStatus === "AtRisk" ? "text-amber-400" : "text-rose-400"}`}
                  >
              ...
              {hasGoals && (
                <div
                  className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 border-slate-950/50 ${totalStatus === "OnTrack" ? "bg-emerald-500" : totalStatus === "AtRisk" ? "bg-amber-500" : "bg-rose-500"}`}
                />
              )}
            </div>
            ...
            {typeof capturePct === "number" && (
              <div className="flex items-center gap-3 border border-white/5 bg-slate-900/50 px-4 py-1.5 rounded-xl shadow-lg">
                <div className="flex flex-col items-end">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-base sm:text-lg font-black tracking-tight ${capturePct === 100 ? "text-emerald-400" : "text-amber-400"}`}
                    >
                      {capturePct}%
                    </span>
                  </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              ...
              >
                <span className="text-sm font-black">
                  {localDecimalPrecision === 2
                    ? ".00"
                    : localDecimalPrecision === 1
                      ? ".0"
                      : "0"}
                </span>
              </button>
        ...
        {selectedItem && (
          <div
            className={`mb-8 ${returnContext ? "rounded-2xl ring-2 ring-cyan-500/40 ring-offset-4 ring-offset-slate-950" : ""}`}
        ...
        {diag.itemsCount === 0 && (
          <div className="py-12 px-6 rounded-3xl bg-slate-900/30 border-2 border-dashed border-slate-800 text-center flex flex-col items-center">
            ...
            <p className="text-slate-400 max-w-md mt-2 mb-6 text-sm">
              Este tablero aún no tiene indicadores asignados.
            </p>
            {isGlobalAdmin && (
              <button
        ...
        {returnContext && activeView === "dashboard" && (
          ...
          >
            ← VOLVER A{" "}
            {returnContext === "control"
              ? "CONTROL"
              : returnContext === "contribution"
                ? "CONTRIBUCIÓN"
                : returnContext === "areas"
                  ? "POR ÁREAS"
                  : returnContext === "plans"
