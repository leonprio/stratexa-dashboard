        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/60 p-6">
          <div className="w-full max-w-xl rounded-xl bg-white p-5 shadow-2xl space-y-4">
            ...
            <div className="max-h-64 overflow-y-auto space-y-3 rounded-lg border border-slate-200 p-2">
              <section><h4 className="px-2 pb-1 text-[10px] font-black uppercase tracking-wider text-slate-500">YA ALINEADOS CON {objective.code}</h4>{pendingDirectKpis.length === 0 && viaCurrentOC.size === 0 ? <p className="p-2 text-xs text-slate-400">Ningún indicador directo alineado.</p> : <>{pendingDirectKpis.map(kpi => { return <div key={kpi.identity} className="flex items-center justify-between rounded-lg bg-slate-50 p-2 text-xs text-slate-700"><strong>{kpi.item?.indicator || kpi.item?.name || 'Indicador'}</strong><button type="button" disabled={loadingOE} onClick={() => removeDirectLogicalKpi(kpi)} className="text-[10px] font-bold text-red-600">QUITAR</button></div>; })}{Array.from(viaCurrentOC).map(key => { const assignment = assignments.find(a => `${a.dashboardId}_${a.itemId}` === key); if (!assignment || removedOcAssignments.includes(assignment.id)) return null; const oc = assignment.contributionObjectiveId ? contributions.find(c => c.id === assignment.contributionObjectiveId) : undefined; const candidate = allKpis.find(k => `${k.dashboard.id}_${k.item.id}` === key); return candidate ? <div key={key} className="flex items-center justify-between rounded-lg bg-indigo-50 p-2 text-xs text-indigo-700"><span><strong>{candidate.item?.indicator || candidate.item?.name || 'Indicador'}</strong><span className="ml-2 text-[10px]">mediante {oc?.displayCode || 'OC'}</span></span><button type="button" onClick={() => removeOcLogicalKpi(assignment.id)} className="text-[10px] font-bold text-red-600">QUITAR</button></div> : null; })}</>}</section>
              <section><h4 className="px-2 pb-1 text-[10px] font-black uppercase tracking-wider text-slate-500">INDICADORES DISPONIBLES PARA ALINEAR</h4>{trulyAvailableKpis.length === 0 ? <p className="p-2 text-xs text-slate-400">No hay indicadores disponibles.</p> : trulyAvailableKpis.map(({ dashboard, item }) => { const key = `${dashboard.id}_${item.id}`; return <label key={key} className="flex items-center gap-2 rounded-lg p-2 text-xs text-slate-700 hover:bg-slate-50"><input type="checkbox" checked={directKpis.includes(key)} onChange={() => setDirectKpis(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key])} /><strong>{item?.indicator || item?.name || 'Indicador'}</strong></label>; })}</section>
            </div>
            {oeError && <p className="text-xs text-red-600">{oeError}</p>}
