...
describe("ContributionExecutiveCell", () => {
  ...
  it("opens the exact KPI from the cell", () => { const navigate = jest.fn(); render(<ContributionExecutiveCell code="OC01" title="Impulsar ventas" kpis={[kpi]} status="BAJO CONTROL" onNavigateToKpi={navigate} />); screen.getByRole("button", { name: "REVISAR KPI" }).click(); expect(navigate).toHaveBeenCalledWith("d", "i"); });
});
