...
export const OperationalControlCenter: React.FC<OperationalControlCenterProps> = ({ dashboards, currentDashboard, globalThresholds, year }) => {
  ...
  const attentionAlerts = alerts.filter(alert => alert.severity !== 'NINGUNO');
  const delayedAlerts = attentionAlerts.filter(alert => alert.missingPeriods > 0 || alert.stalenessDays > 5);
  ...
  return <div className="space-y-8 animate-in fade-in duration-500">
    ...
    <section aria-label="Resumen de control" className="grid grid-cols-1 gap-3 sm:grid-cols-3"><SummaryCard label="Requiere atención" value={attentionAlerts.length} tone="text-rose-400" /><SummaryCard label="Planes activos" value={planSummary.active} tone="text-cyan-400" /><SummaryCard label="Vencidos / atrasados" value={delayedAlerts.length + planSummary.overdue} tone="text-amber-400" /></section>
