...
import { calculateDashboardWeightedScore, getStatusForPercentage } from "../utils/compliance";
    ...
    <div key={(dashboard as any).id} className="space-y-6 animate-in fade-in duration-700 fill-mode-both">
      <div className="DashboardView_Header flex justify-between items-center border-b border-white/5 pb-4 gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-4 glass-panel px-6 py-3 rounded-2xl shadow-2xl">
            <div className="flex flex-col items-end">
              ...
              <span className={`text-4xl font-black tabular-nums leading-none ${totalStatus === 'OnTrack' ? 'text-emerald-400 drop-shadow-[0_0_12px_rgba(16,185,129,0.4)]' : totalStatus === 'AtRisk' ? 'text-amber-400 drop-shadow-[0_0_12px_rgba(245,158,11,0.4)]' : 'text-rose-400 drop-shadow-[0_0_12px_rgba(244,63,94,0.4)]'}`}>
                {Math.round(totalScore)}%
              </span>
            </div>
            <div className="relative">
              <div className={`absolute inset-0 rounded-full blur-[10px] opacity-40 animate-pulse ${totalStatus === 'OnTrack' ? 'bg-emerald-500' : totalStatus === 'AtRisk' ? 'bg-amber-500' : 'bg-rose-500'}`} />
              <div className={`relative w-6 h-6 rounded-full border-2 border-slate-950/50 ${totalStatus === 'OnTrack' ? 'bg-emerald-500' : totalStatus === 'AtRisk' ? 'bg-amber-500' : 'bg-rose-500'}`} />
            </div>
          </div>
          ...
          >
            <span className="text-lg group-hover:rotate-12 transition-transform drop-shadow-lg">✨</span>
          ...
          >
            <span className="text-base">.{localDecimalPrecision === 1 ? '0' : '00'}</span>
          </button>
          ...
          {isAggregate && (isGlobalAdmin || isDirector) && onOpenWeights && (
            <button
      ...
      {(isGlobalAdmin || isDirector) && onUpdateMetadata && (
        <DashboardMetadataModal
          ...
          onSave={async (title, subtitle, group, area) => {
            await onUpdateMetadata!((dashboard as any).id, title, subtitle, group, area);
          }}
