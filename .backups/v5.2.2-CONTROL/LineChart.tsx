...
export const LineChart = ({ progressData, goalData, unit: _unit, type, status, indicator = 'chart', frequency = 'monthly' }: LineChartProps) => {
  ...
  // 🎯 ESCALA DINÁMICA INTELIGENTE (v5.0.0 - FIX)
  // Incluir TODOS los valores válidos (incluyendo 0) para mostrar tendencia completa
  // Solo excluir null/undefined, no valores 0 que son datos reales
  const currentMonthIdx = new Date().getMonth();

  // Filtrar solo hasta el mes actual para evitar proyectar al futuro
  const validPlotData = plotData.filter((d, idx) => idx <= currentMonthIdx && d.value !== null);
  const validGoalData = goalPlotData.filter((d, idx) => idx <= currentMonthIdx && d.value !== null);
  ...
  const minValue = allValues.length > 0 ? Math.min(...allValues.filter(v => v > 0)) : 0;

  // Si el rango es muy pequeño, usar valores absolutos
  // Si hay un valor extremo (>3x el promedio), ajustar escala
  const avgValue = allValues.length > 0 ? allValues.reduce((sum, v) => sum + v, 0) / allValues.length : 100;
  const hasOutlier = maxValue > avgValue * 3;

  const yMax = hasOutlier
    ? avgValue * 2.5  // Limitar escala si hay outlier
    : maxValue * 1.15; // 15% margen superior normal

  const yMin = Math.max(0, minValue * 0.85); // 15% margen inferior, mínimo 0

  // 🛡️ X Domain: Si es semanal, el dominio debe representar el año completo (52 semanas) 
  // o al menos las semanas visibles para que la escala sea consistente.
  const xMaxIdx = isWeekly ? 52 : 11;
